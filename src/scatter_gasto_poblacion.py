...
def load_spending() -> pd.Series:
    ...
    # Intentar con el nombre limpio primero, luego el nombre alternativo
    for nombre in ("agregado_anual_region_clean.parquet", "agregado_anual_region.parquet"):
        parquet = PROCESSED_DIR / nombre
        if parquet.exists():
            break
    if parquet.exists():
        df = pd.read_parquet(parquet)
        # Deduplicar: tomar la fila más completa por región+año
        if "n_licitaciones" in df.columns:
            keys = [c for c in ["RegionUnidad", "region", "anio"] if c in df.columns]
            if keys:
                df = df.loc[df.groupby(keys)["n_licitaciones"].idxmax()]
        # Filtrar outliers de monto (top 3%)
        col_monto = next((c for c in df.columns if "monto" in c.lower()), None)
        if col_monto:
            cap = df[col_monto].quantile(0.97)
            df = df[df[col_monto] <= cap]
        col_monto = next(
        ...
        if col_monto and col_region:
            ...
            # Normalizar nombres completos del parquet → claves de POBLACION
            mapping = {
                # Nombres completos del parquet
                ...
                # Nombres en title-case (fallback)
